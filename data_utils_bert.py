...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer):
        ...
        fin.close()
        fin = open(fname+'.dis_graph', 'rb')
        disgraph = pickle.load(fin)
        fin.close()
        fin = open(fname+'.allsimi', 'rb')
        allsimi = pickle.load(fin)
        fin.close()        
        ...
        for i in range(0, len(lines), 3):
            ...
            #text_bert_indices = tokenizer.text_to_sequence("[CLS] " + context + " [SEP]")
            #aspect_bert_indices = tokenizer.text_to_sequence("[CLS] " + aspect + " [SEP]")
            #left_bert_indices = tokenizer.text_to_sequence("[CLS] " + text_left + " [SEP]")


            dependency_graph = idx2graph[graph_id]
            sentic_graph = idx2graph_s[graph_id]
            sdat_graph = idx2graph_sdat[graph_id]

            idx_dispraph = disgraph[graph_id]
            idx_simi = allsimi[int(graph_id/3)]
            ...
            idx_dispraph = np.pad(disgraph[graph_id], \
                ((0,tokenizer.max_seq_len-disgraph[graph_id].shape[0]),(0,tokenizer.max_seq_len-disgraph[graph_id].shape[0])), 'constant')            
